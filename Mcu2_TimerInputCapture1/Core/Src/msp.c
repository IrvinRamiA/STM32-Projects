...
{
	GPIO_InitTypeDef tim2Gpio;

	// Enable the clock for Timer 2
	__HAL_RCC_TIM2_CLK_ENABLE();
	__HAL_RCC_GPIOA_CLK_ENABLE();

	// Configure a GPIO to behave as Timer 2 Channel 1
	tim2Gpio.Pin = GPIO_PIN_0;
	tim2Gpio.Mode = GPIO_MODE_AF_PP;
	tim2Gpio.Alternate = GPIO_AF1_TIM2;

	HAL_GPIO_Init(GPIOA, &tim2Gpio);
